...
def min1_run(input_list):
	#keep this separate so can be recalled if timing out.
	lib.run_mdrun(*input_list)
	

def eq1_run(eq_list): #same or different list?
	lib.run_mdrun(filename, run_type, dlb_stat, tpr_input, max_hour)


def eq2_run(eq_list): #same or different list?
	lib.run_mdrun(filename, run_type, dlb_stat, tpr_input, max_hour)


def prod1_run(eq_list): #same or different list?
	lib.run_mdrun(filename, run_type, dlb_stat, tpr_input, max_hour)


def prod2_run(eq_list): #same or different list?
	lib.run_mdrun(filename, run_type, dlb_stat, tpr_input, max_hour)
